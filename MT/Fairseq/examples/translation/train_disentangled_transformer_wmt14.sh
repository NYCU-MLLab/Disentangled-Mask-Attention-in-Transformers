...
mi_loss_weight=${mi_loss_weight:-10.}
...
mkdir -p checkpoints/${dataset}/disentangled_transformer
...
PYTHONIOENCODING=utf-8 fairseq-train \
    ...
    --lr ${lr} --lr-scheduler inverse_sqrt --warmup-updates 8000 \
    ...
    --save-dir checkpoints/${dataset}/disentangled_transformer/ckpt \
    --tensorboard-logdir checkpoints/${dataset}/disentangled_transformer/tensorboard \
    --log-format json \
    --keep-last-epochs 5 \
    ...
    --clusters 4 \
    | tee checkpoints/${dataset}/disentangled_transformer/train.log
