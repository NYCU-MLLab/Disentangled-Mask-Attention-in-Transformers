...
from speechbrain.lobes.models.sync_transformer.Transformer import (
    SyncTransformerInterface,
    get_lookahead_mask,
...
class SyncTransformerASR(SyncTransformerInterface):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            causal=causal,
            enc_clusters=enc_clusters,
            dec_clusters=dec_clusters,
    ...
    def forward(
        self, src, tgt, wav_len: Optional[torch.Tensor]=None, pad_idx: int=0,
    ...
    @torch.jit.ignore
    def make_masks(self, src, tgt, wav_len: Optional[torch.Tensor]=None, pad_idx: int=0)-> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    def _init_params(self):
        for p in self.parameters():
            if p.numel() > 1 and p.dim() > 1:
                torch.nn.init.xavier_normal_(p)

        for m in self.modules():
            if isinstance(m, DisentangledMaskAttention):
                torch.nn.init.xavier_normal_(m.linear_semantic_var.weight, gain=0.01)
                torch.nn.init.xavier_normal_(m.linear_head_var.weight, gain=0.01)

                if m.encoder_decoder_attention:
                    torch.nn.init.xavier_normal_(m.linear_semantic_var_k.weight, gain=0.01)
