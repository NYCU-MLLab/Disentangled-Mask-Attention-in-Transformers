...
class SyncTransformerInterface(nn.Module):
    ...
    def __init__(
        ...
        # arguments for disentangled attention
        enc_clusters: int=8,
        dec_clusters: int=4,
        # arguments for sync transformer
        ...
        assert attention_type in ["regularMHA", "RelPosMHAXL", "disentangledMHA"]
        ...
        if num_encoder_layers > 0:
            ...
            if encoder_module == "transformer":
                self.encoder = SyncTransformerEncoder(
                    nhead=nhead,
        ...
        if num_decoder_layers > 0:
            ...
            self.decoder = SyncTransformerDecoder(
...
class SyncTransformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        elif attention_type == "disentangledMHA":
            self.self_att = sb.nnet.attention.DisentangledMaskAttention(
                nhead=nhead,
                d_model=d_model,
                dropout=dropout,
                kdim=kdim,
    ...
    def forward(
        ...
        if return_attn:
            output, self_attn = outputs
        else:
            if isinstance(outputs, Tuple):
                output, self_attn = outputs
            else:
                output, self_attn = outputs, None
...
class SyncTransformerEncoder(nn.Module):
    ...
    def forward(
        ...
        src = src.view(B, L, D, 1).transpose(2, 1) # B x D x L x 1
        src = torch.nn.functional.unfold(
            src,
            kernel_size=(self.window_length, 1),
            stride=self.window_length-self.overlap
        ).view(B, D, W, -1).permute(0, 3, 2, 1).contiguous() # B x C (chunks) x W x D

        C = src.size()[1]
        src = src.view(B*C, W, D)

        if src_key_padding_mask is not None:
            src_key_padding_mask = torch.nn.functional.unfold(
                src_key_padding_mask.view(B, 1, L, 1).float(),
                kernel_size=(self.window_length, 1),
                stride=self.window_length-self.overlap
            ).view(B, W, -1).permute(0, 2, 1).contiguous().bool() # B x C (chunks) x W

            src_key_padding_mask = src_key_padding_mask.view(B*C, W)
            src_key_padding_mask[src_key_padding_mask.logical_not().sum(-1) == 0, 0] = False # prevent from all False cause NaN output in attention
        ...
        for enc_layer in self.layers:
            ...
            if return_attn and attention is not None:
                attention_lst.append(attention)
        output = self.norm(output)

        output = output.view(B, C, W, D)

        return output, attention_lst

class SyncTransformerDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if attention_type == "regularMHA":
        ...
        elif attention_type == "disentnagledMHA":
            self.self_attn = sb.nnet.attention.DisentangledMaskAttention(
                nhead=nhead,
                d_model=d_model,
                kdim=kdim,
                vdim=vdim,
            ...
            self.mutihead_attn = sb.nnet.attention.DisentangledMaskAttention(
                nhead=nhead,
                d_model=d_model,
                kdim=kdim,
                vdim=vdim,
    ...
    def forward(
        ...
        if return_attn:
            tgt2, self_attn = outputs
        else:
            if isinstance(outputs, Tuple):
                tgt2, self_attn = outputs
            else:
                tgt2, self_attn = outputs, None
        ...
        # 2. multi-head attention over the target sequence and encoder states
        if self.normalize_before:
            tgt1 = self.norm2(tgt)
        else:
            tgt1 = tgt

        outputs = self.mutihead_attn(
            query=tgt1,
            key=memory,
            value=memory,
        ...
        if return_attn:
            tgt2, multihead_attention = outputs
        else:
            if isinstance(outputs, Tuple):
                tgt2, multihead_attention = outputs
            else:
                tgt2, multihead_attention = outputs, None
...
class SyncTransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        """
        output = tgt
        self_attns: List[Optional[torch.Tensor]] = []
        multihead_attns: List[Optional[torch.Tensor]] = []
        B, C, W, D = memory.size()

        if memory_key_padding_mask is not None:
            memory_key_padding_mask = torch.nn.functional.unfold(
                memory_key_padding_mask.view(B, 1, -1, 1).float(),
                kernel_size=(self.window_length, 1),
                stride=self.window_length-self.overlap
            ).view(B, W, -1).permute(0, 2, 1).contiguous().bool() # B x C (chunks) x W
            memory_key_padding_mask = memory_key_padding_mask.view(B*C, W)

            # prevent from all False cause NaN output in attention
            memory_key_padding_mask[memory_key_padding_mask.logical_not().sum(-1) == 0, 0] = False

        output = output.view(B, 1, -1, D).repeat(1, C, 1, 1).view(B*C, -1, D)
        if tgt_key_padding_mask is not None:
            tgt_key_padding_mask = tgt_key_padding_mask.view(B, 1, -1).repeat(1, C, 1).view(B*C, -1)
