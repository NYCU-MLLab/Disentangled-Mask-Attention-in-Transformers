...
class ASR(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        logits = self.hparams.ctc_lin(enc_out.float())
        p_ctc = self.hparams.log_softmax(logits.float())

        # output layer for seq2seq log-probabilities
        pred = self.hparams.seq_lin(pred.float())
        p_seq = self.hparams.log_softmax(pred.float())
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        for m in self.hparams.Transformer.modules():
        ...
        loss_kl = torch.cat(loss_kl, dim=0).mean()
        loss_div = torch.cat(loss_div, dim=0).mean()
        loss_mi = torch.cat(loss_mi, dim=0).mean()
        ...
        loss = (
            ...
            + self.hparams.kl_weight * loss_kl
            + self.hparams.div_weight * loss_div
            + self.hparams.mi_weight * loss_mi
        )
        ...
        if self.step % self.hparams.gradient_accumulation == 0:
            self.hparams.summary_writer.add_scalar(summary_prefix+"/loss/loss", loss, global_step=self.iterations)
            self.hparams.summary_writer.add_scalar(summary_prefix+"/loss/loss_kl", loss_kl, global_step=self.iterations)
            self.hparams.summary_writer.add_scalar(summary_prefix+"/loss/loss_div", loss_div, global_step=self.iterations)
            self.hparams.summary_writer.add_scalar(summary_prefix+"/loss/loss_mi", loss_mi, global_step=self.iterations)
            self.hparams.summary_writer.add_scalar(summary_prefix+"/loss/loss_seq", loss_seq, global_step=self.iterations)
            self.hparams.summary_writer.add_scalar(summary_prefix+"/loss/loss_ctc", loss_ctc, global_step=self.iterations)
    ...
    def on_evaluate_start(self, max_key=None, min_key=None):
        ...
        ckpts = self.checkpointer.find_checkpoints(
            max_key=max_key, min_key=min_key, max_num_checkpoints=3
        )
