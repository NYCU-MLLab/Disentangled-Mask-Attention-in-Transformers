...
font = FontProperties(fname=os.path.join(os.path.dirname(os.path.realpath(__file__)), "./NotoSansCJK-Regular.ttc"))
...
def _plot_and_save_attention(att_w, filename, xtokens=None, ytokens=None):
    ...
    for ax, aw in zip(axes, att_w):
        ...
        if xtokens is not None:
            ...
            ax.set_xticklabels(xtokens + [""], rotation=40, fontproperties=font)
        if ytokens is not None:
            ...
            ax.set_yticklabels(ytokens + [""], fontproperties=font)
    fig.tight_layout()
    return fig
